...
<header class="header">
    <div class="header-content">
        <h2>{{ __('Ubah Data Anggota DPR: ' . $anggota->nama_lengkap) }}</h2>
    </div>
</header>
...
<div class="container">
    <div class="card">
        ...
        <a href="{{ route('anggota.index') }}" class="back-link">← Kembali ke Daftar Anggota</a>
        ...
        <form method="POST" action="{{ route('anggota.update', $anggota) }}">
            ...
            <div class="form-group">
                ...
                <input id="nama_depan" name="nama_depan" type="text" class="form-input" value="{{ old('nama_depan', $anggota->nama_depan) }}" required autofocus />
                @error('nama_depan')
            ...
            <div class="form-group">
                ...
                <input id="nama_belakang" name="nama_belakang" type="text" class="form-input" value="{{ old('nama_belakang', $anggota->nama_belakang) }}" />
                @error('nama_belakang')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input id="gelar_depan" name="gelar_depan" type="text" class="form-input" value="{{ old('gelar_depan', $anggota->gelar_depan) }}" />
                @error('gelar_depan')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input id="gelar_belakang" name="gelar_belakang" type="text" class="form-input" value="{{ old('gelar_belakang', $anggota->gelar_belakang) }}" />
            ...
            <div class="form-group">
                ...
                <select id="jabatan" name="jabatan" class="form-select" required>
                    ...
                    @foreach(['Ketua', 'Wakil Ketua', 'Anggota'] as $jabatan)
                        <option value="{{ $jabatan }}" {{ old('jabatan', $anggota->jabatan) == $jabatan ? 'selected' : '' }}>{{ $jabatan }}</option>
                    @endforeach
                </select>
                @error('jabatan')
                    <div class="error-message">{{ $message }}</div>
            ...
            <div class="form-group">
                ...
                <select id="status_pernikahan" name="status_pernikahan" class="form-select" required>
                    ...
                    @foreach(['Kawin', 'Belum Kawin', 'Cerai Hidup', 'Cerai Mati'] as $status)
                        <option value="{{ $status }}" {{ old('status_pernikahan', $anggota->status_pernikahan) == $status ? 'selected' : '' }}>{{ $status }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input id="jumlah_anak" name="jumlah_anak" type="number" min="0" class="form-input" value="{{ old('jumlah_anak', $anggota->jumlah_anak) }}" required />
                @error('jumlah_anak')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
