...
<head>
    ...
    <title>Manajemen Data Penggajian</title>
    <style>
...
<header class="header">
    <div class="header-content">
        <h2>{{ __('Manajemen Data Penggajian (Ringkasan)') }}</h2>
    </div>
</header>
...
<div class="container">
    <div class="card">
        ...
        <div class="header-section">
            <a href="{{ route('payrolls.create') }}" class="btn btn-red">
                Tambah Data Penggajian
            </a>
            
            <form method="GET" action="{{ route('payrolls.index') }}" class="search-form">
                <input id="search" name="search" type="text" class="form-input" placeholder="Cari Anggota..." value="{{ is_array(request('search')) ? '' : request('search') }}" />
            ...
            <tbody>
                @forelse ($payrolls as $item)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('payrolls.show', $item->id_anggota) }}" class="link-detail">
                                {{ $item->nama_lengkap }}
                            </a>
                        </td>
                        <td>{{ $item->jabatan }}</td>
                        ...
                        <td>
                            <a href="{{ route('payrolls.show', $item->id_anggota) }}" class="link-action link-detail" style="color: green;">Detail THP</a>
                            <a href="{{ route('payrolls.edit', $item->id_anggota) }}" class="link-action link-edit">Ubah</a>
                            {{-- Button Hapus menggunakan DOM --}}
                            <form id="delete-payroll-{{ $item->id_anggota }}" action="{{ route('payrolls.destroy', $item->id_anggota) }}" method="POST" class="inline-form">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="link-delete delete-button" data-form-id="delete-payroll-{{ $item->id_anggota }}" data-item-name="{{ $item->nama_lengkap }}" data-module="Penggajian">Hapus</button>
                            </form>
                        </td>
                    </tr>
                @empty
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.delete-button').forEach(button => {
            button.addEventListener('click', function() {
                ...
                modalMessage.textContent = 'Apakah Anda yakin ingin menghapus data ' + moduleName + ' "' + itemName + '"?';
