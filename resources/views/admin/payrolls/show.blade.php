...
<head>
    ...
    <style>
        ...
        .thp-box { background-color: #dc2626; color: white; padding: 1.5rem; border-radius: 0.5rem; text-align: center; }
...
<div class="container">
    <a href="{{ route('payrolls.index') }}" class="back-link">← Kembali ke Daftar Penggajian</a>
            ...
            <tbody>
                @forelse ($component_details as $item)
                    <tr>
                        ...
                        <td style="font-weight: 600; color: {{ $item->nominal_terhitung != $item->nominal_dasar ? '#059669' : '#1f2937' }};">
                            Rp. {{ number_format($item->nominal_terhitung, 0, ',', '.') }}
                            @if ($item->nominal_terhitung != $item->nominal_dasar)
                                <span style="font-weight: 400; font-size: 0.75rem;">(Disesuaikan)</span>
                            @endif
                        </td>
                    </tr>
                @empty
