...
<header class="header">
    <div class="header-content">
        <h2>{{ __('Tambah Komponen Gaji & Tunjangan') }}</h2>
    </div>
</header>
    ...
    <div class="card">
        ...
        <form method="POST" action="{{ route('salary-components.store') }}">
            ...
            <div class="form-group">
                ...
                <input id="nama_komponen" name="nama_komponen" type="text" class="form-input" value="{{ old('nama_komponen') }}" required autofocus />
                @error('nama_komponen')
            ...
            <div class="form-group">
                ...
                <select id="jabatan" name="jabatan" class="form-select" required>
                    ...
                    @foreach($positions as $position)
                        <option value="{{ $position }}" {{ old('jabatan') == $position ? 'selected' : '' }}>{{ $position }}</option>
                    @endforeach
                </select>
                @error('jabatan')
                    <div class="error-message">{{ $message }}</div>
            ...
            <div class="form-group">
                ...
                <input id="nominal" name="nominal" type="number" step="0.01" class="form-input" value="{{ old('nominal') }}" required />
                @error('nominal')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
