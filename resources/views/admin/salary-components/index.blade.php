    ...
    <div class="card">
        ...
        <div class="header-section">
            ...
            <form method="GET" action="{{ route('salary-components.index') }}" class="search-form">
                <input id="search" name="search" type="text" class="form-input" placeholder="Cari Nama, Kategori, Jabatan, Nominal..." value="{{ is_array(request('search')) ? '' : request('search') }}" />
            ...
            <tbody>
                @forelse ($components as $item)
                    <tr>
                        ...
                        <td>{{ $item->kategori }}</td>
                        <td>{{ $item->jabatan }}</td>
                        <td>Rp. {{ number_format((float)$item->nominal, 2, ',', '.') }}</td>
                        <td>{{ $item->satuan }}</td>
                        <td>
                            <a href="{{ route('salary-components.edit', $item->id_komponen_gaji) }}" class="link-action link-edit">Ubah</a>
                            {{-- Button Hapus menggunakan DOM --}}
                            <form id="delete-component-{{ $item->id_komponen_gaji }}" action="{{ route('salary-components.destroy', $item->id_komponen_gaji) }}" method="POST" class="inline-form">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="link-delete delete-button" data-form-id="delete-component-{{ $item->id_komponen_gaji }}" data-item-name="{{ $item->nama_komponen }}" data-module="Komponen Gaji">Hapus</button>
                            </form>
                        </td>
                    </tr>
                @empty
